...
- **◐ Phase 3 — deep-agent fan-out — FIRST PASS done (143/2,091 = 6.8%), FULL RUN NOW IN FLIGHT.** First pass (286 agents): **49 confirmed / 94 refuted / 22 no-verdict — ~66% of verdicts refuted.** The refutation breakdown is **not "low precision" but three distinct, fixable problems**: **44% size-bias** (big IG names like Enbridge trip size-correlated signatures), **~40% corpus data-artifacts** (equity/shelf counted as debt, mis-sizing), **11% stale-but-real** (debt since forgiven/refinanced). Confirmed fragility is cross-sector, not AI: Bausch, Xerox, Lexmark, SunPower, Celularity, Workhorse, Hyperscale Data; X.AI the lone AI in the high tier. **The remaining 1,945 scored entities are now being deep-dived in paced waves** (`scripts/build_full_deepdive_targets.py`) — ending the top-143 sample.
- **◐ Phase 4 — synthesis is PROVISIONAL** → [SYNTHESIS_2026-06-13.md](SYNTHESIS_2026-06-13.md) (based on 143/2,091; superseded by the full run). **Orchestrator EDGAR verification** ([survivor_filing_verification.md](survivor_filing_verification.md)): top **9/9** survivors carry filing-tier distress (keyword-tier). **XBRL net-debt join DONE** ([large_levered_net_leverage.md](large_levered_net_leverage.md)): separates real distress (coverage <1.5×: Arbor, Blackstone Mortgage, MFA, Liberty Global — **mortgage-REIT-dominated**) from refi-risk (Bausch is cov 1.88× → **refi-risk, correcting the agent's "distressed" label**), and quantifies the size-bias: **47/92 large-levered names were "scan_oversized"** (gross flag >3× actual net debt). Provisional headline: no single epicenter; attrition not imminent cascade (p_real ≈ 0.39, window not cracked) — *pending full coverage*.
- **✅ Bounded free-public-tier goal COMPLETE (2026-06-13).** Economy-wide XBRL scan over **all 7,992 SEC filers** done; **450 deep-verified → 174 confirmed; 124/166 orchestrator-filing-verified** (75% agent reliability); contagion across all 3 graphs (capital/contract/LEI); multi-year trajectories (~62% worsening); **5 off-EDGAR sources ingested** (EDGAR, FDIC 4,352 banks, CAISO + EIA power layer, GDELT news); integrated verdict + navigable index. See [ATLAS_INDEX.md](ATLAS_INDEX.md), [SYNTHESIS_2026-06-13.md](SYNTHESIS_2026-06-13.md), [boundary_ledger.md](boundary_ledger.md).
- **⛔ Out of scope as documented real walls** (not the bounded goal; would resume the *unbounded* ambition only with operator resources): paywalled sources (PACER/NAIC/customs/ratings/transcripts), account-gated free keys (FRED/PatentsView), JS/anti-bot ISO/WARN scraping, the private/non-filer universe toward "low millions," full §2.8 distress extraction, international. Each tagged real-wall vs open-work in the boundary ledger.

> **Caveat (binding) + boundary note:** filing-verified *by the orchestrator* = the **top 9 names, keyword-tier**; everything else is agent-asserted. The synthesis is a **first pass over 6.8% of the scored set** (itself a deal-corpus slice of the ~790k enumerated, itself a slice of the true millions) — a calibrated **direction-setter, not a verdict.** **Real limit hit (2026-06-13):** API **concurrency/throughput** rate-limiting caps sustainable parallelism at ~16 concurrent agents — a *speed* ceiling, **not a scope** ceiling (same coverage reachable serially over more wall-clock). Running scope vs real boundary is tracked in [boundary_ledger.md](boundary_ledger.md).
