...
- **✅ Phase 3 — deep-agent fan-out** (`scripts/build_phase3_targets.py` → 143 targets; `scripts/workflows/phase3_deep_dive.workflow.js`; extractor `scripts/extract_phase3_findings.py` → [phase3_findings.md](phase3_findings.md), [phase3_plan.md](phase3_plan.md)). 286 agents, 10.7M tokens. **49 confirmed / 94 refuted / 22 no-verdict — ~66% of verdicts refuted as data artifacts or stale signals; the scan is high-recall, low-precision and the verification layer is load-bearing.** Confirmed fragility is cross-sector, not AI: already-distressed incl. Bausch, Xerox, Lexmark, SunPower, Celularity, Perfect Moment, Workhorse, Hyperscale Data; X.AI the lone AI name in the high tier.
- **✅ Phase 4 — synthesis + forward projection** → [SYNTHESIS_2026-06-13.md](SYNTHESIS_2026-06-13.md). Plus **orchestrator EDGAR verification** ([survivor_filing_verification.md](survivor_filing_verification.md)): top **9/9** survivors carry filing-tier distress (keyword-tier). Headline: **no single epicenter** — distributed fragility, a wide already-failing small-cap tail + a large-levered layer on the 2025–27 refi wall; AI is one stressed cohort among several; projection is attrition, not imminent cascade (verdict-tree p_real ≈ 0.39, funding window not yet cracked).
- **⏳ Queued refinements** (cleared by storage, not yet run): XBRL net-debt/EBITDA join (notional → true leverage), Form-4/8-K/NT **distress §2.8 extraction** (the one unpopulated signature), deepening past the first 143 into the ~1,000-name canary tail, international beyond US-primary.

> **Caveat (binding for anyone reading the results):** filing-verified *by the orchestrator* = the **top 9 distress names only, keyword-tier** (distress language + NT flag, not a full numeric re-audit); everything else is agent-asserted. The ~66% artifact/stale rate means the raw scan is ⅔ noise — trust survivors-after-verification, never the raw flag count. A high-recall first pass over ~0.02% of the universe: a calibrated **direction-setter**, not a proven verdict.
