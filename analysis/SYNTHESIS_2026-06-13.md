...
> **⚠️ PROVISIONAL — SUPERSEDED IN PROGRESS.** This was written on a **first pass of 143 of 2,091 scored entities (6.8%)** — itself only the deal-corpus slice of the ~790k enumerated universe. Calling it a characterization "of the economy" is **overreach**, conceded as such. The full deep-dive of all 2,091 is now running in paced waves, and the XBRL net-debt join has already **corrected** parts of this (e.g. Bausch is refi-risk, not "already-distressed"; true filing-tier distress is mortgage-REIT-concentrated). Read this as a **direction-setter from a pilot**, not a verdict. Scope vs real boundary: [boundary_ledger.md](boundary_ledger.md).

**2026-06-13.** First-pass read: a sector-agnostic signature scan over **2,091 scored entities**, **143 deep-agent forensic profiles + adversarial refutations**, **9 orchestrator-filing-verified** survivors, and an XBRL net-debt join on the large-levered layer. The narrow defensible claim is *"in the names looked at so far, fragility is distributed and AI isn't dominant"* — not a closed verdict over the economy. See *Confidence & limits*.
