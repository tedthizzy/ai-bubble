...
| 2b | Covenant *threshold* + current headroom | **DARK** | redacted in the filed amendment ("CONFIDENTIAL INFORMATION… [redacted]") |
...
- **Exactly two things stay genuinely DARK** — **CoreWeave's covenant headroom** (redacted) and **whether the OpenAI/Microsoft take-or-pay survives a customer funding failure** (undisclosed) — plus one market datum, **real neocloud occupancy %**. These live in a credit-committee room and a contract data-room and are obtainable only by a counterparty source.
...
**So the timing reduces to a single question the DARK items would answer:** *when do the held-at-par marks get forced into recognition?* The three triggers, all now sharper: a **CoreWeave covenant event** (the amendments show the headroom is already being managed); the **2027 GPU-resale normalization** (Blackwell glut + Rubin) hitting a cohort whose book lives the secondary market has already repudiated; or an **OpenAI/Oracle funding event** (the CDS is already moving). The structure is damning and witnessed; the fuse length is held in the two rooms we can't see — which is the most precise version of the answer to what omniscience would buy you: *not the diagnosis, only the date.*
