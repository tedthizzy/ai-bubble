...
> **⚓ Frame — read first (reanchor):** this repo is a **general forensic engine for financial fragility & mispricing across the _whole (US-primary) economy_**, **no sector prior**. **AI / data-center is _case zero_, not the object** (cf. fiber 1999, shale 2014 in the base rates). **Maximum exhaustiveness — broad AND deep, uncapped, ≥ $1M-substance floor; acting resource-constrained is NEVER correct; the only stop is physics.** Full scope: **[analysis/total_ecosystem_dive.md](analysis/total_ecosystem_dive.md)**. *Much of the dated, AI-specific program below is **case zero's** workplan — the verdict, the signals, the Q4 adjudication — and remains valid as the first deployment; the generalization to the whole economy is WS5 (now active, ungated, uncapped).*
