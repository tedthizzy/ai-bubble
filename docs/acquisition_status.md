...
Last updated: 2026-06-02 07:54 UTC.
...
- Latest evidence-gated report: `data/reports/BURRY_REPORT_EvidenceGated_20260602-0754.md`
...
- Automated materiality adjudication decisions: 6,663 decisions, 6,463 with
  ...
  sources, 4,508 supported as material blockers, 2,155 requiring deeper
  extraction, 0 requiring source retrieval, and 2,955 source-backed rows
  approved for metric use. Those approved rows total $8.470T as row-level
  ...
  disclosures to $4.498T across 1,603 metric groups; they are not treated as
...
- Semantic metric-validity gating now scans approved materiality rows as a
  separate source-text dimension: 2,720 approved rows classify as committed
  debt/obligation text, 235 remain indeterminate review candidates, and 0
...
- Decision coverage over packaged blocker groups: 100.0%; unresolved decision
  share remains 32.34% (still extraction-bound, not source-retrieval-bound).
- Top remaining decision gaps are now named counterparty role extraction
  ...
  asset/UPB/financing-capacity splitting (150), specific committed-obligation
  semantic confirmation (58), queue/permit/interconnection linkage (43),
...
- Materiality decisions: 4,508 source-supported blockers, 2,155 requiring
  deeper extraction, 0 requiring source retrieval, and 2,955 rows approved
  for metric use (6,463 quote-backed decisions plus 200 row-context-backed
  decisions on non-text sources).
- Automated row-level supported amount approved for metric use: $8.470T.
- Deduped automated final metric support: $4.498T across 1,603
...
- Semantic hard flags remaining in approved metric rows: 0
  asset/capacity-or-boilerplate rows; 235 indeterminate rows remain queued for
